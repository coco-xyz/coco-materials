...
| [Microsoft Teams](#ms-teams) | 已支持 | 企业团队、Microsoft 365 集成 |
| [Zalo](#zalo) | 已支持 | 越南用户、Zalo Bot 平台 |
| [Zalo Personal](#zalo-personal) | 已支持 | 越南用户、个人账号、功能齐全 |
...
## 方案 G：Microsoft Teams 部署 {#ms-teams}

**预计用时：10-15 分钟**

> **说明：** Microsoft Teams 使用基于 **Webhook 的消息端点**——你的 COCO 实例需要一个公网 HTTPS URL。连接时 COCO Dashboard 会自动处理。

需要 3 个凭证：

| 凭证 | 来源 | 说明 |
|------|------|------|
| App ID | Azure 门户 → 应用注册 → 应用程序（客户端）ID | 机器人唯一标识 |
| App Password | Azure 门户 → 应用注册 → 证书和密码 | 客户端密钥值 |
| Tenant ID | Azure 门户 → 应用注册 → 目录（租户）ID | 你的组织租户 |

### 第 1 步：创建 Azure 应用注册

1. 前往 [Azure 门户](https://portal.azure.com) → **Microsoft Entra ID** → **应用注册**
2. 点击 **新注册**
3. 输入名称（例如 `COCO AI Employee`）
4. **支持的帐户类型**选择**单租户**
5. **重定向 URI** 留空
6. 点击 **注册**
7. 记录 **应用程序（客户端）ID** 和 **目录（租户）ID**

### 第 2 步：创建客户端密钥

1. 在应用注册中，前往 **证书和密码**
2. 点击 **新客户端密码**
3. 输入描述（例如 `coco-bot-secret`），选择过期时间
4. 点击 **添加**
5. **立即复制密钥值**——创建后仅显示一次

> **重要：** 密钥值创建后仅显示一次。如果丢失，需要创建新密钥。

### 第 3 步：添加 Graph API 权限（可选但推荐）

Graph API 权限可解锁完整功能集（聊天历史、文件下载、表情回应）。

1. 在应用注册中，前往 **清单（Manifest）** 选项卡
2. 找到 `"requiredResourceAccess"` 数组并替换为：
...
3. 点击 **保存**
4. 前往 **API 权限**，点击 **代表 [你的租户] 授予管理员同意**
5. 确认所有权限显示绿色勾选

| 权限 | 类型 | 用途 |
|------|------|------|
| `Chat.Read.All` | 应用程序 | 读取私信和群聊历史及附件 |
| `ChannelMessage.Read.All` | 应用程序 | 读取频道消息历史及附件 |
| `Files.Read.All` | 应用程序 | 从 OneDrive/SharePoint 下载文件 |
| `User.Read.All` | 应用程序 | 解析用户提及和搜索用户 |
| `Chat.ReadWrite` | 委派 | 发送表情回应（思考中指示） |
| `ChannelMessage.Send` | 委派 | 在频道中发送表情回应 |
| `ChannelMessage.Read.All` | 委派 | 从频道下载图片/文件 |
| `Files.Read.All` | 委派 | 代表用户下载共享文件 |
| `offline_access` | 委派 | 通过刷新令牌保持委派令牌有效 |

> **注意：** 管理员同意需要租户管理员权限。基本消息功能不需要这些权限即可使用。

### 第 4 步：在 COCO Dashboard 中连接

1. 登录 [COCO Dashboard](https://coco.xyz/dashboard)
2. 进入 AI 员工实例详情页 → **会话入口** → 点击 **Microsoft Teams** 连接按钮
3. 输入 **App ID**、**App Password** 和 **Tenant ID**
4. 点击 **Connect**——系统自动配置机器人端点
...
```json
{
  ...
  "description": {
    "short": "Teams AI 员工",
    "full": "COCO AI 数字员工——你在 Microsoft Teams 上的 AI 助手。"
  },
...
将 `<你的 App ID>`（两处）替换为第 1 步中的应用程序（客户端）ID。

### 第 6 步：打包并安装

1. 创建两个图标文件：`color.png`（192×192）和 `outline.png`（32×32）
2. 将 `manifest.json` 和两个图标放在同一文件夹，压缩为 `.zip` 文件
3. 在 Microsoft Teams 中 → **应用** → **管理你的应用** → **上传自定义应用**
4. 选择 `.zip` 文件，点击 **添加**

> **注意：** 旁加载需要启用"上传自定义应用"策略。如果看不到上传选项，请联系你的 Teams 管理员。

### 第 7 步：开始使用

1. 在 Teams 中搜索你的机器人名称（例如 "COCO AI"）
2. 发起私信对话——AI 员工立即回复
3. 如需在频道中使用，安装时将机器人添加到团队
4. 部署完成！
...
| 问题 | 解决方案 |
|------|----------|
| Bot 不响应 | 确认 App ID、App Password 和 Tenant ID 是否正确，检查消息端点是否可达 |
| 搜索时找不到应用 | 确保清单已正确打包并旁加载。完全退出并重启 Teams |
| 文件附件返回 401 错误 | 需要授予 Graph API 权限的管理员同意（第 3 步） |
| Bot 在群组/频道中不可见 | Bot 必须先被添加到团队——安装时选择"添加到团队" |
| Graph 功能不工作 | 所有三个凭证都必须配置，且必须授予管理员同意 |
...
> **说明：** Zalo 是越南最流行的即时通讯平台。Zalo Bot 平台使用官方 API，默认采用**轮询模式**——无需公网 URL 或服务器。

只需 1 个凭证：

| 凭证 | 格式 | 说明 |
|------|------|------|
| Bot Token | `numeric_id:secret` | 从 Zalo Bot 平台开发者门户获取 |
...
> **重要：** Bot Token 是机器人的唯一凭证，请勿泄露给他人。

### 第 2 步：在 COCO Dashboard 中连接

1. 登录 [COCO Dashboard](https://coco.xyz/dashboard)
2. 进入 AI 员工实例详情页 → **会话入口** → 点击 **Zalo** 连接按钮
...
> **提示：** Zalo Bot 默认使用轮询模式——无需公网 URL 或服务器。如需实时投递，可选配 Webhook 模式（需要公网 HTTPS URL）。

### Zalo 常见问题

| 问题 | 解决方案 |
|------|----------|
| Bot 不响应 | 确认 Bot Token 是否正确，检查 Dashboard 中的连接状态 |
| 响应缓慢 | 轮询模式有轻微延迟——可考虑 Webhook 模式以实现实时投递 |
| 无法添加到群组 | 群组访问需要显式配置——请让 AI 员工启用群组权限 |
| 图片发送失败 | 发送图片需要公网 URL——不支持本地文件路径 |

---

## 方案 I：Zalo Personal 部署 {#zalo-personal}

**预计用时：约 5 分钟**

> **说明：** Zalo Personal 通过真实 Zalo 账号使用**二维码扫描**连接，类似 WhatsApp 部署。相比 Bot 平台，它提供更丰富的功能——完整的群聊支持、文件分享、贴纸和表情回应。
...
1. 登录 [COCO Dashboard](https://coco.xyz/dashboard)
2. 进入 AI 员工实例详情页 → **会话入口** → 点击 **Zalo Personal** 连接按钮
3. 系统生成二维码（可能需要约 30 秒）
...
默认只有 Owner（管理员）可以和 Bot 对话。如需允许其他人使用，有两种模式：

**Allowlist 模式**——仅指定用户可对话：

> 对 Bot 发送：`打开 allowlist 模式，将用户 123456 加入到列表中`

**Open 模式**——所有人都可对话：

> 对 Bot 发送：`打开 open 模式，任何人都可以 DM 你`

### Zalo Personal 与 Zalo Bot 平台对比

| 功能 | Zalo Personal | Zalo Bot 平台 |
|------|---------------|---------------|
| 配置方式 | 扫描二维码 | Bot Token |
| 群聊 | 完整支持 | 仅允许列表 |
| 文件和 PDF | 支持 | 不支持 |
| 表情回应 | 支持（11 种） | 不支持 |
| 图片 | 文件发送/接收 | 仅 URL |
...
| 问题 | 解决方案 |
|------|----------|
| Bot 掉线 | Zalo 同时只允许一个网页端会话——如果在浏览器中打开了 Zalo Web，Bot 会断开。从 Dashboard 重新扫码即可 |
| 二维码不显示 | 首次生成可能需要 30–60 秒，请勿关闭页面 |
| Bot 在群聊中不响应 | 确认 Bot 账号已被添加到群组，并且被 @mention |
| 会话过期 | 如果手机长时间离线，Zalo 会话可能过期。从 Dashboard 扫描新二维码即可 |
| 其他人无法发消息 | 默认只有 Owner 可以聊天。启用 Allowlist 或 Open 模式即可 |
