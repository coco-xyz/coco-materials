...
<img :src="withBase('/wecom-config-table.png')" alt="企业微信对接需要的5个配置项：企业ID、应用Secret、AgentId、Token、EncodingAESKey" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />

### 第1步：获取企业ID（WECOM_CORP_ID）

访问企业微信管理后台 **我的企业** 页面，在页面底部找到 **企业ID**。

<img :src="withBase('/wecom-get-corp-id.png')" alt="我的企业页面 — 在页面底部找到企业ID" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />
...
<img :src="withBase('/wecom-create-app.png')" alt="应用管理页面 — 在「自建」区域点击「创建应用」" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />

<br>

### 第3步：设置应用名称和可见范围

给 Bot 起一个好记的名字，选择该应用对企业成员的 **可见范围**（决定哪些成员可以看到并使用这个 Bot）。

<img :src="withBase('/wecom-app-name.png')" alt="创建应用 — 填写应用名称，设置可见范围" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />

1. 上传应用 Logo（建议 750×750 px）
2. 填写应用名称（如 `COCO AI员工`）
3. 设置 **可见范围** — 选择可以使用该应用的部门/成员
4. 点击 **创建应用**
...
<img :src="withBase('/wecom-agent-secret.png')" alt="应用详情页 — 获取AgentId和Secret，点击「设置API接收」" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />

> **重要：** Secret 只显示一次，请立即保存。

<br>

### 第5步：获取 Token 和 EncodingAESKey

在「API接收消息」配置页面：

- **URL** — 填写回调地址（如 `https://your-domain.com/wecom/webhook`）
- **Token** — 点击「随机获取」生成
- **EncodingAESKey** — 点击「随机获取」生成

<img :src="withBase('/wecom-token-aeskey.png')" alt="API接收消息配置 — 填写URL、获取Token和EncodingAESKey" style="max-width: 620px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />

> **注意：** 填写 URL 前需确保服务端已部署并能正确响应企业微信的验证请求，否则保存会失败。

<br>

### 第6步：在COCO平台绑定

> **免费试用期提示：** 在免费试用期间，支付和自动化配置步骤将被跳过。完成上述企业微信端配置后，请将以下信息提供给 COCO 技术团队，我们将人工协助完成配置，并在 **24小时内** 为你完成部署。

请将以下5项信息提供给 COCO 团队：

| 字段 | 获取位置 |
|------|---------|
| **企业ID**（CorpID） | 管理后台 → 我的企业 → 企业ID |
| **AgentId** | 应用管理 → 你的应用 → 应用详情页 |
| **Secret** | 应用管理 → 你的应用 → 获取Secret |
| **Token** | 应用管理 → 你的应用 → 接收消息 → API配置 |
| **EncodingAESKey** | 应用管理 → 你的应用 → 接收消息 → API配置 |

<!--
1. 登录 [COCO Dashboard](https://coco.site/dashboard)
2. 进入 **Connect** 或 **Channels** 页面
3. 选择 **企业微信**
4. 填入以下信息：
...
| 字段 | 来源 |
|------|------|
| 企业ID（CorpID） | 企业微信管理后台 → 我的企业 → 企业ID |
| AgentId | 企业微信管理后台 → 应用管理 → 应用详情 |
| Secret | 企业微信管理后台 → 应用管理 → 应用详情 |
| Token | 企业微信管理后台 → 应用管理 → 接收消息 |
| EncodingAESKey | 企业微信管理后台 → 应用管理 → 接收消息 |
...
COCO 团队完成配置后：
1. 打开企业微信，在工作台找到你创建的应用
2. 向应用发送任意消息，AI员工会立即响应
...
| 问题 | 解决方案 |
|------|---------|
| 无法保存 API 配置 | 确保 COCO 后端已部署并正常响应 — 企业微信在保存时会验证 URL 有效性 |
| 应用对部分成员不可见 | 检查应用管理中的 **可见范围** 设置，确保所有使用者都已添加 |
| 群聊中机器人不响应 | 确认应用已添加至群组，且群成员在可见范围内 |
| Secret 丢失或忘记 | 在应用管理中重新点击 **获取Secret** — 原 Secret 立即失效，将新 Secret 提供给 COCO 团队 |
