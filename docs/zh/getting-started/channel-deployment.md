...
#### 第6步：获取 Verification Token（仅 Webhook 模式需要）
...
#### 第8步：配置事件订阅（仅 Webhook 模式需要）

> **WebSocket 用户可跳过此步。** WebSocket 长连接模式下，事件通过长连接自动接收，无需配置 Webhook URL 和事件订阅。
