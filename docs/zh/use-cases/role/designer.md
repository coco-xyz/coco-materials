...
> 整理设计评审会议结构，将多位评审者的反馈综合为可执行的优先列表，并识别需要解决的相互矛盾反馈。

::: details 痛点与 COCO 解决方案

**痛点：设计评审反馈不一致、相互矛盾，难以付诸行动**

设计评审会议产生的反馈往往难以付诸行动。评论的范围从主观偏好到客观可用性问题不等，两者之间没有区分。来自不同利益相关者的矛盾反馈让设计师陷入僵局。当反馈来自多个渠道——Figma 评论、会议记录、Slack 消息——且在不同时间到达时，综合挑战更加复杂。

**COCO 的解决方案**

1. **反馈收集与结构化**：COCO 将原始评审反馈整理为类别（可用性、视觉、业务、技术），并区分事实性观察与主观偏好。
2. **冲突检测**：COCO 识别来自不同评审者的矛盾反馈，标记需要设计决策的冲突。
3. **优先级排序**：COCO 应用严重程度和可行性标准，按实施优先级对反馈进行排序。
4. **回复起草**：COCO 为每条反馈起草回复——无论是设计变更计划，还是不实施的文档化理由。
5. **评审会议促进**：COCO 生成结构化的评审议程和评估标准，使现场评审会议更有成效。

:::

::: details 预期成果与影响

- **评审会议效率**：有 AI 生成议程的结构化评审在相同时间内产生 **2 倍的可行反馈**
- **反馈响应率**：使用 AI 辅助综合的设计师对 **95%+** 的反馈作出响应（vs. 无结构追踪时 **40–60%**）
- **冲突解决时间**：自动检测并呈现反馈冲突以供决策，从数天压缩至 **当场解决**
- **利益相关者满意度**：含文档化理由的结构化反馈回复使利益相关者对设计流程的满意度 **提高 35%**
- **设计迭代速度**：清晰的优先反馈列表将平均迭代周期从 **3–5 天缩短至 1–2 天**
...
**提示词 1：设计评审反馈综合器**
```
请综合以下设计评审反馈并创建优先行动清单。

被评审的设计：[描述——功能、界面、流程、组件]
设计阶段：[早期概念/中保真/最终评审]

收到的反馈（来自多位评审者）：
[粘贴所有反馈——每条评论注明来源角色，例如"产品经理："、"UX 主管："、"工程师:"、"利益相关者:"]

综合：
1. 可用性问题（影响用户完成目标的能力）——附严重程度列表
2. 视觉/美学反馈（外观、品牌一致性、精打磨程度）——列表
3. 业务需求问题（功能、边缘场景）——列表
4. 技术可行性问题——列表
5. 需要设计决策的冲突反馈——识别冲突及相关利益相关者
6. 优先行动清单：关键/高优先/锦上添花
7. 需要推迟或降低优先级的反馈，附文档化理由
```
...
设计：[描述]
留下反馈的平台：[Figma / Miro / 文档 / 邮件 / Slack]

反馈评论（注明评论者角色和评论内容）：
[例如：
"产品经理：CTA 太小了，用户会忽略"
"UX 主管：整体层级不错，但次要操作在视觉上与主要操作竞争"
"工程师：在此场景中，我们无法在不造成显著性能影响的情况下实现模糊效果"
"利益相关者：我更喜欢红色按钮，更符合我们的品牌气质"]

对每条评论：
1. 类别：可用性/视觉/业务/技术/偏好
2. 有效性评估：有效问题/主观偏好/事实误解/需要更多信息
3. 设计回应：将做出的变更，或不做变更的文档化理由
4. 发送给评论者的回复消息（专业、具体、表示感谢）

另请指出：任何相互矛盾需要在回复前做出设计决策的评论。
```
