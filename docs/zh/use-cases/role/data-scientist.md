...
> 分析原始数据集和现有特征，推荐高潜力的派生特征、变换和交互项——附带预期预测价值的依据说明。

::: details 痛点与 COCO 解决方案

**痛点：特征工程是机器学习开发中价值最高、最耗时的步骤**

特征工程对模型性能的影响一贯超过算法选择——但它也是机器学习开发过程中最耗时、最依赖知识的部分。一名熟练的数据科学家可能会将 60–70% 的项目时间花在特征创建、选择和验证上。对于经验较少的从业者而言，挑战更为艰巨：知道尝试哪些变换、创建哪些交互项以及如何将领域知识编码为特征，需要多年的积累。

**COCO 的解决方案**

1. **数据集分析**：COCO 检查原始特征分布、数据类型、缺失值模式和相关性，以理解数据全貌。
2. **领域感知特征建议**：COCO 生成基于领域知识（金融、医疗、零售等）的特征工程建议。
3. **变换推荐**：COCO 根据分布特征推荐适当的变换（对数、多项式、归一化、分箱）。
4. **交互项识别**：COCO 基于领域逻辑和统计相关模式识别潜在有价值的交互项。
5. **实现代码生成**：COCO 为每个推荐特征生成 Python/Pandas 代码，供数据科学家审阅并纳入特征管道。

:::

::: details 预期成果与影响

- **特征工程时间**：AI 辅助推荐将典型表格数据集的探索时间减少 **40–60%**
- **模型性能提升**：使用 AI 建议特征的团队比人工基线在主要指标上 **提升 5–15%**
- **初级数据科学家效果**：初级从业者使用 AI 推荐在 **2–3 个月内** 接近高级从业者的特征质量水平（vs. 靠经验积累需要 **12–18 个月**）
- **特征覆盖率**：AI 探索生成的候选特征比人工方法 **多 3–5 倍**
- **代码质量**：AI 生成的特征实现代码减少管道错误，提高可复现性
...
问题：[描述预测任务——例如客户流失预测、欺诈检测]
目标变量：[二分类/多分类——描述]
预测时间窗口：[例如 30 天流失]

可用的原始特征：[列出特征名称、类型和描述]
领域背景：[描述业务背景——是什么驱动了目标行为]

推荐：
1. 从日期/时间戳列衍生的时序特征
2. 比率和速率特征（例如每活跃天支持工单数）
3. 行为轨迹特征（比较近期与历史活动的趋势指标）
4. 领域特定的交互项（附理由）
5. 高基数特征的分类编码建议
6. 可考虑的外部数据源特征

对每个推荐：特征名称、构建逻辑和预期预测价值依据。
```

**提示词 2：特征管道代码生成器**
```
请为以下特征规范生成 Python 特征工程代码。

数据集：[描述原始数据结构]
目标特征（附构建逻辑）：
[例如：
- days_since_last_purchase：last_purchase_date 到快照日期的天数
- purchase_frequency_90d：近 90 天购买次数 / 90
- high_value_customer_flag：avg_order_value > $500 时为 1，否则为 0]

请生成干净、可生产部署的 Python/Pandas 代码：
1. 在单个函数中创建所有指定特征
2. 处理每个特征的空值和边缘情况
3. 添加适当的类型转换
4. 为每个特征添加文档字符串
5. 遵循 pandas 性能最佳实践（避免 iterrows，使用向量化操作）
6. 为每个特征变换添加简单的单元测试
```
