...
1. **Hook Pattern Analysis and Classification**: Identifies and classifies viral hook types from high-performing content across platforms.
...
2. **Multi-Variant Hook Generation**: Produces 8-15 distinct hook variants for any content piece, each using a different structural pattern.
...
3. **Hook A/B Test Framework**: Structures systematic hook testing to build a proprietary dataset of what works for a specific brand voice and audience.
...
4. **Content-to-Hook Reverse Engineering**: Analyzes existing content pieces that underperformed and diagnoses why the hook failed.
...
5. **Trending Topic Hook Integration**: Monitors trending discussions and identifies opportunities to connect brand content to high-traffic conversations with relevant hooks.
...
- Your top 3 recommendations and why
```

**Prompt 2: Diagnose and Rewrite an Underperforming Post**
```
...
6. What should I do with this content now — repost with a new hook? Archive it? Turn it into a different content format?
```

**Prompt 3: Platform-Specific Hook Library for a Content Theme**
```
...
5. What are 3 hook formats currently trending in my topic area that I should experiment with in the next 30 days?
```

**Prompt 4: Trending Topic Hook Generator**
```
...
5. What format would work best for this content (short-form text, video, carousel, thread) and why?
```

**Prompt 5: 30-Day Hook Testing Sprint Plan**
```
...
1. **Voice Capture and Profiling**: Analyzes existing posts, emails, and transcripts to build an authentic voice model for each executive.
...
2. **Insight Extraction from Raw Input**: Converts rough ideas, voice memos, meeting notes, and bullet points into structured content.
...
3. **Format and Algorithm Optimization**: Structures posts for maximum LinkedIn algorithmic performance.
...
4. **Content Calendar Integration**: Maintains a consistent publishing cadence across multiple executives simultaneously.
...
5. **Performance Analysis and Iteration**: Tracks post performance and continuously improves content based on what actually works.
...
6. **Multi-Executive Coordination**: Manages thought leadership programs for 3-10+ executives simultaneously without confusion.
...
5. Flag any sections that sound too corporate and suggest more authentic alternatives
```

**Prompt 2: Build a 30-Day Thought Leadership Calendar**
```
...
6. Generate first drafts for the first week's posts so we can start publishing immediately
```

**Prompt 3: Develop an Executive Voice Profile**
```
...
5. Write 1 sample LinkedIn post that demonstrates their voice authentically — something that could have come from them
```

**Prompt 4: Repurpose Long-Form Content into LinkedIn Posts**
```
...
6. Suggest how to connect the posts as a series without making it feel like a forced promotional campaign
```

**Prompt 5: Write an Engagement-Optimized Opinion Post**
```
...
1. **Community Health Assessment and Strategy**: Evaluates current community state and builds a data-informed engagement strategy.
...
2. **Daily Engagement Content Generation**: Produces the full spectrum of community content at high volume and quality.
...
3. **Power Member Identification and Nurturing**: Identifies high-value community members and creates personalized nurture strategies.
...
4. **User-Generated Content Amplification**: Surfaces and amplifies the best member-created content across community channels and external platforms.
...
5. **Product Feedback Loop Integration**: Connects community conversations to the product roadmap and customer success teams.
...
5. Define 3 metrics I should track weekly to know if this plan is working
```

**Prompt 2: Design a Power Member Activation Program**
```
...
6. How do I keep ambassadors motivated and recognized without burning them out?
```

**Prompt 3: Generate a Community Onboarding Sequence**
```
...
6. What should I do if a new member still hasn't posted after 14 days?
```

**Prompt 4: Synthesize Community Feedback for Product Teams**
```
...
5. Write a community response post that closes the feedback loop with members: "We heard you, and here's what we're doing about it"
```

**Prompt 5: Write Community Event Content and Follow-Up**
```
...
1. **Creator Research and Profile Analysis**: Builds a comprehensive understanding of each creator before writing a single word of outreach.
...
2. **Hyper-Personalized Outreach Draft Generation**: Writes outreach messages that demonstrate genuine familiarity with each creator's unique work.
...
3. **Campaign Brief and Collaboration Deck Creation**: Develops the full suite of partnership materials after initial response.
...
4. **Follow-Up Sequence Management**: Manages the timing and tone of follow-up communications without damaging relationship potential.
...
5. **Contract and Rate Negotiation Support**: Provides frameworks and language for compensation discussions.
...
5. Provide a follow-up version if they don't respond within 7 days
```

**Prompt 2: Build a Campaign Outreach List Prioritization Framework**
```
...
6. What are the 3 biggest risks in this creator mix and how do I mitigate them?
```

**Prompt 3: Write a Campaign Brief for a Confirmed Creator**
```
...
5. Add a "creative examples" section showing what we love about their previous content to anchor the direction
```

**Prompt 4: Negotiate a Creator Rate**
```
...
5. What concessions could we offer besides cash to close a deal with a creator who values our brand but finds the rate low? (early access, co-creation credit, performance bonuses, etc.)
```

**Prompt 5: Build a Creator Relationship Nurture Sequence**
```
...
1. **ICP and Persona Framework Development**: Defines exactly who the launch is for and why, with the specificity that makes sales and marketing execution precise.
...
2. **Positioning and Messaging Architecture**: Creates the messaging framework that will be used consistently across every channel and by every team member.
...
3. **Launch Execution Timeline and Owner Assignment**: Builds the operational plan that actually makes the launch happen on schedule.
...
4. **Sales Enablement Package Creation**: Produces the materials the sales team needs to sell the new product effectively from day one.
...
5. **Launch Metrics and Feedback Loop Design**: Defines how success is measured and how the team learns and adjusts in the first 90 days.
...
6. Launch success metrics with definitions and 30/60/90-day targets
```

**Prompt 2: Develop a Competitive Positioning Framework**
```
...
5. What "trap questions" should we ask early in discovery to understand which competitor we're up against?
```

**Prompt 3: Write a Sales Enablement Launch Package**
```
...
6. Draft the launch kickoff email to the sales team: what's launching, why it matters to their quota, and what they need to do this week
```

**Prompt 4: Design a Launch Metrics Dashboard**
```
...
6. How do we separate "launch execution quality" from "product-market fit quality" in the metrics? (i.e., how do we know if a bad launch result means the product is wrong vs. the launch was poorly executed?)
```

**Prompt 5: Build a Channel Partner GTM Activation Plan**
```
...
1. **Competitor Content Pattern Analysis**: COCO structures the analysis of competitor social content across key dimensions.
...
2. **Engagement Signal Interpretation**: COCO analyzes audience response data to identify what resonates.
...
3. **Messaging Theme Extraction**: COCO identifies the core positioning messages competitors are actively pushing.
...
4. **Campaign and Initiative Detection**: COCO identifies when competitors are running coordinated content campaigns.
...
5. **Positioning Gap Analysis**: COCO compares competitor positioning to identify unclaimed territory.
...
6. **Actionable Strategic Recommendation Output**: COCO converts competitive intelligence into a prioritized action list.
...
6. Identify 3 positioning whitespace opportunities for us based on what competitors are NOT doing
```

**Prompt 2: Competitor Messaging Shift Detection**
```
...
5. Recommend a specific response: should we adjust our messaging? Claim the territory they vacated? Defend our position?
```

**Prompt 3: Competitor Campaign Reverse-Engineering**
```
...
6. Suggest a content or campaign idea inspired by this analysis that fits our positioning
```

**Prompt 4: Weekly Competitive Social Listening Report**
```
...
5. Watch list for next week: what should I specifically look for?
```

**Prompt 5: Positioning Whitespace Analysis**
```
...
1. **Platform-Specific Hashtag Strategy**: COCO generates hashtag recommendations calibrated to each platform's algorithm and best practices.
...
2. **Reach vs. Competition Balance Analysis**: COCO analyzes the tradeoff between hashtag reach and saturation level.
...
3. **YouTube SEO Keyword Optimization**: COCO generates keyword-optimized titles, descriptions, and chapter structures.
...
4. **Content Title and Headline SEO Optimization**: COCO optimizes article and LinkedIn post titles for discoverability.
...
5. **Trending Topic Integration**: COCO identifies relevant trending topics and hashtags to incorporate into content.
...
6. **Hashtag and Keyword Strategy Refresh Reviews**: COCO conducts periodic strategy audits to keep distribution optimization current.
...
5. Suggest 2 alternative hashtag combinations if I want to test different audience targeting
```

**Prompt 2: Instagram Hashtag Mix Generator**
```
...
5. Optimal hashtag placement recommendation (caption vs. first comment)
```

**Prompt 3: YouTube Video SEO Package**
```
...
6. First comment draft with additional keywords for SEO
```

**Prompt 4: Content Title A/B Test Generator**
```
...
5. What secondary keywords to include in the content body for SEO coverage
```

**Prompt 5: Quarterly Hashtag and Keyword Strategy Review**
```
...
1. **Rapid Statement Drafting**: COCO generates crisis response statements in multiple variants for fast internal selection.
...
2. **Tone Calibration Framework**: COCO analyzes the crisis type and recommends the appropriate tone register.
...
3. **Stakeholder Communication Cascade**: COCO drafts the full suite of stakeholder-specific communications.
...
4. **FAQ and Anticipated Question Preparation**: COCO prepares the team for the questions they will face.
...
5. **Communication Timeline Planning**: COCO structures the staged communication rollout.
...
6. **Narrative Control and Recovery Arc**: COCO helps design the story the brand wants to tell after the immediate crisis subsides.
...
5. Recommended publishing sequence (what goes where and in what order)
```

**Prompt 2: Full Stakeholder Communication Cascade**
```
...
6. A shared "facts sheet" all communications can reference for consistency
```

**Prompt 3: Crisis FAQ Preparation**
```
...
5. 3 questions that could trap us if answered poorly, with guidance on how to handle them safely
```

**Prompt 4: Crisis Communication Timeline Plan**
```
...
6. "Green light" criteria: what conditions need to be met before each communication stage launches?
```

**Prompt 5: Post-Crisis Recovery Narrative**
```
