...
| [Zalo Personal (Unofficial)](#zalo-personal) | 已支持 | 越南用户、个人 Zalo 账号（非官方） |
...
<img :src="withBase('/feishu-add-bot-capability.png')" alt="点击添加机器人卡片" style="max-width: 520px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />
...
<a href="https://coco.xyz/dashboard" style="color: #f5a623; font-weight: 600;">登录 Dashboard</a> → 对话入口 → 选择微信连接 → 待二维码生成 → 手机微信扫码即可
...
> **说明：** Microsoft Teams 是企业广泛使用的 Microsoft 365 协作平台。部署需要创建 Azure App Registration 和 Bot 资源。无需服务器或编程技能——Azure 免费层即可满足。

对接 Microsoft Teams 需要 **3 个配置项**：

| 配置项 | 获取位置 | 说明 |
|--------|---------|------|
| App ID | Azure Portal → App Registration → Application (client) ID | 标识你的机器人应用 |
| App Password | Azure Portal → App Registration → Certificates & secrets → Secret Value | 认证你的机器人 |
| Tenant ID | Azure Portal → App Registration → Directory (tenant) ID | 仅单租户机器人需要 |

### 第1步：创建 Azure App Registration

1. 打开 [Azure Portal](https://portal.azure.com) → **Microsoft Entra ID** → **App registrations**
2. 点击 **New registration**
3. 输入名称（如 `COCO AI Employee`）
4. 在 **Supported account types** 下选择：
   - **Single tenant** — 仅你的组织中的用户（推荐企业使用）
   - **Multitenant** — 任何组织中的用户
5. **Redirect URI** 留空
6. 点击 **Register**

创建完成后记录：
- **Application (client) ID** — 即你的 **App ID**
- **Directory (tenant) ID** — 单租户时需要

### 第2步：创建 Client Secret（客户端密钥）

1. 在 App Registration 页面，进入 **Certificates & secrets**
2. 点击 **New client secret**
3. 输入描述（如 `COCO Bot Secret`），选择过期时间
4. 点击 **Add**
5. **立即复制 Secret 的 Value** — 离开此页面后无法再次查看

> **注意：** 客户端密钥仅在创建时显示一次，请立即安全保存。

### 第3步：配置 Graph API 权限

在 App Registration 页面，进入 **API permissions** → **Add a permission** → **Microsoft Graph**。

#### 应用程序权限（Application Permissions）

选择 **Application permissions**，添加以下权限：

| 权限 | 用途 |
|------|------|
| `Files.Read.All` | 从 OneDrive/SharePoint 下载文件 |
| `Chat.Read.All` | 读取私聊和群聊历史记录 |
| `ChannelMessage.Read.All` | 读取团队频道消息历史 |
| `User.Read.All` | 解析用户提及和搜索用户 |

#### 委托权限（Delegated Permissions）

返回 **Add a permission** → **Microsoft Graph** → **Delegated permissions**，添加以下权限：

| 权限 | 用途 |
|------|------|
| `Chat.ReadWrite` | 代表用户读取和发送聊天消息 |
| `ChannelMessage.Send` | 代表用户发送频道消息 |
| `ChannelMessage.Read.All` | 代表用户读取频道消息 |
| `Files.Read.All` | 访问用户有权访问的文件 |
| `offline_access` | 在用户未主动登录时保持访问权限 |

#### 授予管理员同意

全部 9 项权限（4 项应用程序权限 + 5 项委托权限）添加完成后，点击 **Grant admin consent for [你的组织]** 并确认。所有权限应显示绿色勾号。

> **注意：** 必须获得管理员同意。否则文件下载、聊天历史、表情回应和智能模式功能将无法使用。

### 第4步：创建 Azure Bot 资源

1. 在 Azure Portal 中搜索 **Azure Bot** 并点击 **Create**
2. 填写：
   - **Bot handle**：唯一名称（如 `coco-ai-employee`）
   - **Pricing tier**：F0（免费）
   - **Microsoft App ID**：选择 **Use existing app registration**，输入第1步的 **App ID**
3. 点击 **Review + create**，然后点击 **Create**
4. 部署完成后，进入 Bot 资源 → **Configuration**
5. **Messaging endpoint** 暂时留空 — 在 Dashboard 连接后再填写
...
| Tenant ID | Azure Portal → App Registration → Directory (tenant) ID（仅单租户） |

5. 点击 **连接** — 系统将验证凭据并部署渠道

> **提示：** 如果你的机器人是多租户模式，Tenant ID 字段可以留空。

### 第6步：在 Azure 中设置 Messaging Endpoint

连接成功后，员工详情页会显示一个 **Messaging Endpoint** URL。复制该 URL。

1. 返回 Azure Portal → 你的 **Azure Bot** 资源 → **Configuration**
2. 将 URL 粘贴到 **Messaging endpoint** 字段
3. 点击 **Apply**

> **注意：** 此步骤必须完成。没有 Messaging endpoint，Azure Bot Service 无法将 Teams 消息转发给你的 AI 员工。

### 第7步：创建并安装 Teams App Manifest

要让机器人在 Teams 中可用，需要一个 Teams App Manifest — 包含 `manifest.json` 和两个图标文件的 `.zip` 压缩包。

我们提供了包含 COCO 图标的现成模板，下载以下三个文件：

- [manifest.json](/ms-teams-manifest/manifest.json) — 应用 manifest 模板
- [color.png](/ms-teams-manifest/color.png) — 192x192 全彩应用图标
- [outline.png](/ms-teams-manifest/outline.png) — 32x32 透明轮廓图标

::: warning 压缩前请编辑 manifest.json
用任意文本编辑器打开 `manifest.json`，将所有占位符替换为你的实际值。
:::
...
| 占位符 | 替换为 | 位置 |
|--------|-------|------|
| `<YOUR_APP_ID>` | 第1步获取的 App ID | `id`、`botId`、`webApplicationInfo.id`（共 3 处） |
| `<YOUR_COMPANY_NAME>` | 你的公司或团队名称 | `developer.name` |
| `<YOUR_BOT_DISPLAY_NAME>` | 在 Teams 中显示的 Bot 名称（如 `COCO AI 员工`） | `name.short` |
| `<SHORT_DESCRIPTION>` | 一句话简介（如 `AI 智能助手`） | `description.short` |
| `<FULL_DESCRIPTION>` | 详细描述 | `description.full` |

> **关于 scopes 字段：** `scopes` 定义 Bot 可以在哪些场景中使用：`personal` = 1对1私聊，`team` = 团队频道，`groupChat` = 群聊。默认三个都已启用。`resourceSpecific` 权限启用资源特定同意（RSC），允许在安装了机器人的聊天和频道中读取消息。

编辑完 `manifest.json` 后，选中这三个文件压缩为一个 `.zip` 文件。

**在 Teams 中安装：**
- **侧载安装（测试）：** Teams → Apps → Manage your apps → Upload a custom app → 选择 `.zip` 文件
- **管理员部署（全组织）：** Teams Admin Center → Manage apps → Upload new app → 选择 `.zip` 文件，然后分配给用户/用户组

### 第8步：复制 App Catalog ID

安装应用后，您需要复制其目录 ID：

1. 打开 [Teams 管理中心](https://admin.teams.microsoft.com/policies/manage-apps)
2. 按名称搜索您的应用
3. 点击应用打开详情页
4. 复制详情页上显示的 **App ID**（这是目录 ID，与 Azure 应用注册 ID 不同）
5. 返回 COCO Dashboard，将其粘贴到 **App Catalog ID** 字段中
...
| Teams 中看不到机器人 | Teams App Manifest 必须已安装（侧载或管理员部署）。检查 manifest.json 中的 App ID 是否与 Azure App Registration 一致 |
...
## 选项J：Zalo Personal (Unofficial) 部署 {#zalo-personal}

**预计耗时：约2分钟**

> **说明：** Zalo Personal (Unofficial) 通过二维码扫描使用真实 Zalo 账号连接——无需注册 Bot、无需 API 密钥、无需开发者账号。它通过 [zca-js](https://github.com/nicenathapong/zca-js)（逆向工程协议库）实现连接。

::: warning 非官方协议
Zalo Personal (Unofficial) 使用的是非官方的逆向工程协议（zca-js），**未经 Zalo/VNG 官方认可**。你的账号可能会被 Zalo 限制或封禁。**使用风险自负。** 如需官方集成，请使用 [Zalo Bot Platform](#zalo)。
:::
...
### 第1步：让你的 AI 员工安装 Zalo Personal (Unofficial)

在任何已连接的渠道（Telegram、飞书、WhatsApp 等）中，给你的 AI 员工发送消息：

> "安装 Zalo Personal (Unofficial) 渠道"

AI 员工会自动安装 Zalo Personal (Unofficial) 组件并生成二维码，通过聊天发送给你。
...
### Zalo Personal (Unofficial) 常见问题

| 问题 | 解决方案 |
|------|----------|
| 二维码过期未扫描 | 让 AI 员工重新生成二维码（如："重新连接 Zalo Personal (Unofficial)"） |
| 连接一段时间后断开 | Zalo 可能会断开长时间运行的会话。让 AI 员工重新连接（如："重新连接 Zalo Personal (Unofficial)"） |
...
| 想要断开连接 | 让 AI 员工断开 Zalo Personal (Unofficial) 渠道（如："断开 Zalo Personal (Unofficial)"） |

<!-- 已注释：之前基于 Dashboard 的设置流程（已替换为上方的 AI 员工引导流程——Zalo Personal (Unofficial) 不再通过 COCO Dashboard 界面配置）
...
### 第1步：点击 Zalo Personal (Unofficial) 卡片的「连接」

1. 登录 [COCO Dashboard](https://coco.xyz/dashboard)
2. 进入员工实例详情页
3. 找到 **Zalo Personal (Unofficial)** 卡片，点击 **连接**
4. 系统将生成二维码——可能需要几秒钟
...
1. 扫码成功后，Dashboard 自动检测到连接——Zalo Personal (Unofficial) 卡片显示为 **已连接（Connected）**
...
| 想要断开连接 | 在员工详情页的 Zalo Personal (Unofficial) 卡片上点击 **断开连接** 按钮 |
