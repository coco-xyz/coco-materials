...
::: warning 压缩前请替换 YOUR_APP_ID
用任意文本编辑器打开 `manifest.json`，将其中所有 `<YOUR_APP_ID>` 占位符（共 3 处）替换为第1步中获取的实际 App ID。
:::

> **说明：** `<YOUR_APP_ID>` 出现在三个字段中：`id`、`botId` 和 `webApplicationInfo.id`。`webApplicationInfo` 部分启用 Graph API 集成。`resourceSpecific` 权限启用资源特定同意（RSC），允许在安装了机器人的聊天和频道中读取消息。
