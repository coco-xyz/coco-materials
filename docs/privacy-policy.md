...
description: COCO AI Privacy Policy — how we collect, use, share, and protect your personal information.
aside: false
editLink: false
lastUpdated: false
---
...
- Name, email address, and login credentials (authenticated via Logto SSO)
- Company name, job title, and industry (during onboarding)
- Subscription plan selection (Air / Pro / Ultra)
...
- Knowledge base content you upload or connect
- Channel configuration (which Telegram groups, Lark chats, Slack workspaces your employee is deployed to)

### 1.4 Conversation & Interaction Data

When you or your team interact with your AI digital employee:

- Messages sent to and received from your digital employee
- Files, images, and documents shared in conversations
- Task execution logs (actions taken, tools invoked, outputs generated)
- Agent-to-agent communication logs (when your digital employee collaborates with other agents via HXA-Connect)

### 1.5 Persistent Memory Data
...
|----------|--------|---------|
| **Anthropic** | Claude (Sonnet, Opus, Haiku) | Reasoning, conversation, task execution |

We may add providers in the future. This list will be updated accordingly, and existing users will be notified of any changes.
...
- Our LLM provider (Anthropic) is **contractually prohibited** from using your data for model training or improvement
- Data sent to Anthropic's API is processed solely to generate a response to your specific request, with **zero data retention** by the provider — your content is not logged, stored, or reviewed by Anthropic
- There is **no scenario** under which opting out is required — non-training is the default and only mode

This commitment applies equally to all subscription tiers (Air, Pro, Ultra) and all data types (conversations, files, memory, configurations).
...
3. The relevant content is routed to Anthropic's API via encrypted connection (TLS 1.2+)
4. Anthropic processes the request, generates a response, and returns it — **no data is retained on Anthropic's servers**
...
These logs are accessible to workspace administrators and subject to the retention periods in Section 5.
...
| Authentication | Logto | SSO and user authentication |
| LLM Provider | Anthropic | AI processing (see Section 3) |
...
We retain your information only as long as necessary, with specific timeframes:

| Data Type | Retention Period |
|-----------|-----------------|
| Account & profile information | Duration of account + 90 days after deletion |
| Conversation & task logs | Duration of account (Enterprise: configurable by workspace admin) |
| Digital employee memory | Duration of account; clearable by you at any time |
| Digital employee configuration | Duration of account + 30 days after deletion |
| Payment & billing records | 7 years (legal/tax requirements) |
| Usage & analytics data (aggregated) | 2 years |
| Security & access logs | 1 year |
| Marketing communication records | 3 years from last interaction |

**Account deletion:** When you cancel your subscription, your dedicated cloud instance and all associated data (conversations, memory, configuration, files) are destroyed through our automated lifecycle management. Account data is retained only for the periods specified above, then permanently deleted or irreversibly anonymized.

**No indefinite retention:** Unlike some competitors, we do not retain your data indefinitely in any form after the specified retention periods expire.
...
- **Essential Cookies:** Required for Dashboard functionality — authentication sessions, subscription state, and channel configuration. Cannot be disabled.
- **Analytics Cookies:** Used only with your consent where required by law, to understand feature adoption and improve the Platform.
...
We respect Do Not Track (DNT) browser signals. When we detect a DNT signal, we disable non-essential tracking.
...
- **Access control:** Role-based permissions (user / builder / admin) via the Dashboard; SSO authentication via Logto
...
For transfers of personal data from the European Economic Area (EEA), United Kingdom, or Switzerland to countries without an adequate level of data protection, we rely on:
...
- Enterprise data processing is governed by a separate **Data Processing Agreement (DPA)**, available upon request at service@coco.xyz
...
- **Material changes:** We will notify you via email and Dashboard notification at least 30 days before changes take effect, clearly describing what has changed
