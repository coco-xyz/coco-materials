...
要让机器人在 Teams 中可用，需要创建 Teams App Manifest：

::: warning 以下 manifest 不能直接复制使用
你必须将其中所有 `<YOUR_APP_ID>` 占位符（共 3 处）替换为第1步中获取的实际 App ID，然后再打包上传。
:::
...
> **说明：** 将 `<YOUR_APP_ID>` 替换为你的实际 App ID（`id`、`botId`、`webApplicationInfo.id` 三处）。`webApplicationInfo` 部分启用 Graph API 集成。`resourceSpecific` 权限启用资源特定同意（RSC），允许在安装了机器人的聊天和频道中读取消息。

2. 添加两个图标文件：
   - **color.png** — 192x192 像素，全彩应用图标
   - **outline.png** — 32x32 像素，透明轮廓图标
3. 将三个文件压缩为一个 `.zip` 文件
