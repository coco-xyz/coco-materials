...
<img :src="withBase('/feishu-permissions.png')" alt="权限管理页面 — 点击「开通权限」或「批量导入/导出权限」" style="max-width: 520px; width: 100%; border-radius: 8px; margin: 0.5rem 0;" />
...
5. 部署完成后，页面会显示你的专属 **Webhook URL**，复制此 URL（下一步需要用到）
...
7. 部署完成后，页面会显示你的专属 **Webhook URL**，复制此 URL（下一步需要用到）
...
在应用管理页面，进入左侧 **权限管理**。你可以点击 **开通权限** 逐个选择需要的权限，也可以点击 **批量导入/导出权限** 粘贴我们准备好的 JSON 一键导入：
