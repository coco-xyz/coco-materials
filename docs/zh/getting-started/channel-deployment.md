...
3. 设置 **可见范围** — 选择可以使用该应用的团队成员
4. 点击 **创建应用**
5. 创建成功后，应用详情页面会显示：
   - **AgentId** — 复制并保存
   - 点击 **获取Secret** 获取 **Secret** — 复制并保存
...
1. 在管理后台左侧菜单，进入 **我的企业**
2. 滚动到页面底部，找到 **企业ID**（即 CorpID）
3. 复制并保存此值

### 第4步：配置接收消息 API

1. 返回 **应用管理** → 进入你创建的应用
2. 向下滚动找到 **接收消息** 板块，点击 **设置API接收**
3. 在 API 配置页面，需要填写以下信息：
   - **URL** — 你的 COCO Webhook 地址（见第5步）
   - **Token** — 点击 **随机获取** 并复制该值
   - **EncodingAESKey** — 点击 **随机获取** 并复制该值（固定43位字符）

> **重要：** 在点击保存之前，必须确保 COCO 后端已部署并能响应企业微信的验证请求。企业微信在保存时会立即验证 URL 的有效性，如果服务未就绪则配置会失败。
